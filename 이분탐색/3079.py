...
세 번째 사람이 그곳으로 이동해서 심사를 받으면 된다. 10초가 되는 순간, 
네 번째 사람이 이곳으로 이동해서 심사를 받으면 되고, 14초가 되었을 때는 다섯 번째 사람이 첫 번째 심사대로 이동해서 심사를 받으면 된다. 
...
n, m = map(int, sys.stdin.readline().split()) # n개의 입국 심사대, m명의 사람
t = [int(sys.stdin.readline()) for _ in range(n)] #각 입국 심사대마다 걸리는 시간
...
while start <= end:
    mid = (start + end) // 2 #중간 값 (총 시간)
    cnt = 0 #명 수    

    for i in range(n):
        cnt += (mid // t[i])
        #jubge_time + time과 time과 비교해서 jubge_time이 다음 시간인 time보다 더 작다면 

    if cnt >= m: #명수가 같거나 많다면 시간이 길다고 판단
        end = mid -1
        result = mid
    else:
        start = mid + 1
